    ...
    def steer(self, frame, lane_lines):
        ...
        if new_steering_angle == -90:
            ...
            if self.car is not None:
                self.car.drive(self.curr_steering_angle)
...
def detect_edges(frame):
   ...
   # show_image("blue mask", mask)
    ...
    thres = cv2.adaptiveThreshold(gray,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY_INV,191,20)
    mask = thres
...
def region_of_interest(canny):
    ...
    polygon = np.array([[
        (0, height * 1 / 2 - padding),
        (width, height * 1 / 2-padding),
        (width, height),
        (0, height),
    ]], np.int32)
...
def test_video(video_file):
    lane_follower = HandCodedLaneFollower(car=myCar)
